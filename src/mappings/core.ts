...
function isCompleteMint(mintId: string): boolean {
  const ev = MintEvent.load(mintId)
  if (ev === null) {
    return false
  }
  return ev.sender !== null // sufficient checks
}
...
export function handleTransfer(event: Transfer): void {
  ...
  // this code seems to assume pair will never be null, but is that really true?
  // the load function is autogenerated
  if (pair === null) {
    return
  }
...
export function handleMint(event: Mint): void {
  ...
  let amountTotalUSD = t1D
    .times(token1Amount)
    .plus(t0D.times(token0Amount))
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountETH = t1D
    .times(amount1Total)
    .plus(t0D.times(amount0Total))
